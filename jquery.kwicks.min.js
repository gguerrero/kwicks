 ...
 */(function($){var methods={init:function(opts){var defaults={maxSize:-1,minSize:-1,spacing:5,duration:500,isVertical:false,easing:undefined,behavior:null,autoResize:true,showSpeed:undefined};var o=$.extend(defaults,opts);if(o.minSize!==-1&&o.maxSize!==-1)
...
$(window).on('resize',onResize);};Kwick.prototype.initSlideShow=function(){if(!this.opts.showSpeed||this.opts.behavior!=="slideshow")return;if(isNaN(this.opts.showSpeed)){throw new Error('Invalid slideShow option (not a number): '+this.opts.slideShow);}
var self=this,speed=parseInt(this.opts.showSpeed)*1000,numSlides=this.$panels.length,curSlide=0;clearInterval(this.slideShowInterval);this.slideShowInterval=setInterval(function(){self.expand(curSlide++%numSlides);},speed);};Kwick.prototype.getContainerSize=function(clearCache){var containerSize=this._containerSize;if(clearCache||!containerSize){containerSize=this._containerSize=this.$container[this.primaryDimension]();}
